...
namespace UDS.SubModule.UnitiveDocument.Document
{
	...
	public class BrowseDocument : System.Web.UI.Page
	{
		...
		protected void ShowBodyDetail()
		{
			...
			try
			{
				dataReader = doc.GetDocDetail(DocID,UserCookie.Value);
			}
			catch
			{
			...
			{  
				...
				if(dataReader["FileVisualPath"].ToString()!="")
				{
					if(dataReader["FileVisualPath"].ToString().Substring(0,4)=="Mail")
						 this.lblFileVisualPath.Text+= "&nbsp;<a href='Download.aspx?destFileName=\\"+Server.UrlEncode(dataReader["FileVisualPath"].ToString())+"'>"+dataReader["FileName"].ToString()+"</a><br>";
						//this.lblFileVisualPath.Text += "<a href='../"+dataReader[21].ToString()+"'>"+dataReader[18].ToString()+"</a><br>&nbsp;&nbsp;";
					else
						this.lblFileVisualPath.Text += "<a href='Download.aspx?destFileName=\\Document\\"+Server.UrlEncode(dataReader["FileVisualPath"].ToString())+"'>"+dataReader["FileName"].ToString()+"</a><br>&nbsp;&nbsp;";
				}
			}
